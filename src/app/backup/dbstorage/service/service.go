...
func (s *TaskService) getFormConfig(env *authstub.Env) (config proto.TaskConfig, err error) {
	if config.FilterPose, err = proto.GetValidFilter(env.Req.FormValue("filter_pose")); err != nil {
		err = proto.ErrInvalidFilterPose
		return
	}

	if config.FilterBlur, err = proto.GetValidFilter(env.Req.FormValue("filter_blur")); err != nil {
		err = proto.ErrInvalidFilterBlur
		return
	}

	if config.FilterCover, err = proto.GetValidFilter(env.Req.FormValue("filter_cover")); err != nil {
		err = proto.ErrInvalidFilterCover
		return
	}

	if config.MinWidth, err = proto.GetValidSizeLimit(env.Req.FormValue("min_width")); err != nil {
		err = proto.ErrInvalidWidth
		return
	}

	if config.MinHeight, err = proto.GetValidSizeLimit(env.Req.FormValue("min_height")); err != nil {
		err = proto.ErrInvalidHeight
		return
	}
...
func (s *TaskService) PostUpload(ctx context.Context, env *authstub.Env) (*UploadFaceRespItem, error) {
	...
	if err != nil {
		xl.Errorf("upload => parse config error: %s", err)
		return nil, err
	}
	...
	if err != nil {
		xl.Error("image is not uploaded : ", err)
	...
	if err != nil {
		xl.Error("read uploaded image err : ", err)
		return nil, proto.ErrInvalidFile
	}
	imgBase64 := proto.BASE64_PREFIX + base64.StdEncoding.EncodeToString(content)

	//init face group service
	faceGroup := outer.NewFaceGroup(
	...
	id, err := faceGroup.Add(ctx, config, proto.GroupName(groupName), "", proto.ImageURI(imgBase64), "", "")
	if err != nil {
		xl.Errorf("call faceGroup Add error: %s", err)
		return nil, err
	}

	return &UploadFaceRespItem{Id: id}, nil
}
...
func (s *TaskService) PostTask_Start(ctx context.Context, args *struct{ CmdArgs []string }, env *authstub.Env) error {
	...
	if task.Status == proto.STARTED || task.Status == proto.PENDING || task.Status == proto.STOPPING {
...
func (s *TaskService) PostTask_Stop(ctx context.Context, args *struct{ CmdArgs []string }, env *authstub.Env) error {
	...
	} else if task.Status != proto.STARTED && task.Status != proto.PENDING {
