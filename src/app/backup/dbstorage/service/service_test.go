...
func Test_GetFormConfig(t *testing.T) {
	...
	env.Req.Form["filter_pose"] = []string{"abc"}
	_, err := s.getFormConfig(env)
	assert.Equal(t, proto.ErrInvalidFilterPose, err)

	env.Req.Form["filter_pose"] = []string{"1"}
	env.Req.Form["filter_blur"] = []string{"abc"}
	_, err = s.getFormConfig(env)
	assert.Equal(t, proto.ErrInvalidFilterBlur, err)

	env.Req.Form["filter_blur"] = []string{"0"}
	env.Req.Form["filter_cover"] = []string{"abc"}
	_, err = s.getFormConfig(env)
	assert.Equal(t, proto.ErrInvalidFilterCover, err)

	env.Req.Form["filter_cover"] = []string{"1"}
	env.Req.Form["min_width"] = []string{"abc"}
	_, err = s.getFormConfig(env)
	assert.Equal(t, proto.ErrInvalidWidth, err)

	env.Req.Form["min_width"] = []string{"10"}
	env.Req.Form["min_height"] = []string{"abc"}
	_, err = s.getFormConfig(env)
	assert.Equal(t, proto.ErrInvalidHeight, err)

	env.Req.Form["min_height"] = []string{"70"}
	config, err := s.getFormConfig(env)
	assert.Nil(t, err)
	assert.Equal(t, true, config.FilterPose)
	assert.Equal(t, false, config.FilterBlur)
	assert.Equal(t, true, config.FilterCover)
	assert.Equal(t, proto.MIN_FACE_SIZE, config.MinWidth)
	assert.Equal(t, 70, config.MinHeight)
}
...
func Test_GetTask(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t, proto.STARTED, task.Status)
